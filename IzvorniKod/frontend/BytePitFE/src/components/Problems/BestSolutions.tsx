...
const BestSolutions: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `/api/solutions/get/best/${zadatakId}`,
          options
        );
        if (!response.ok) {
        ...
        const solvedResponse = await fetch(
          `/api/solutions/solved/${zadatakId}`,
          options
        );
  ...
  const downloadSolution = async (rbr: BigInt, natjecatelj: string) => {
    try {
      const credentials = btoa(`${user.korisnickoIme}:${user.lozinka}`);
      const options = {
        method: "GET",
      ...
      const response = await fetch(
        `/api/solutions/code?rbr=${rbr}&zadatak=${zadatakId}&natjecatelj=${natjecatelj}`,
        options
      );
      if (!response.ok) {
        window.alert(
          "Morate riješiti zadatak da biste vidjeli rješenja drugih korisnika!"
        );
      ...
      formattedSolutionCode = formattedSolutionCode.substring(
        1,
        formattedSolutionCode.length - 1
      );
    ...
    ? [
        ...
        },
        {
          ...
          render: (record: RjesenjeInfo) => (
            <Button
              className="download"
              onClick={() => downloadSolution(record.rBr, record.korisnickoIme)}
							disabled={user.uloga != "NATJECATELJ"}
            >
              <span>Dohvati</span>
            </Button>
          ),
